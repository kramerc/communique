...
function Connection(client, server, options) {
  ...
  // Handle introduction replies
  var introListener = function (message) {
    message.params.splice(0, 1);
    this.writeToServerBuffer(message.params.join(' '));
  }.bind(this);

  this.on('RPL_WELCOME', introListener)
      .on('RPL_YOURHOST', introListener)
      .on('RPL_CREATED', introListener)
      .on('RPL_MYINFO', introListener)
      .on('RPL_ISUPPORT', introListener);
